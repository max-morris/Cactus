...
variable \verb|$VARIABLE| at the time \texttt{gmake <\var{config
name}>-config} runs. The \verb|$| sign can be escaped by duplicating it
...
Makefile, \verb|${HOME}| when \texttt{<\var{config name}>-config} runs,
...
\item [\texttt{CROSS\_COMPILE}] Enables cross compilation.
  ...
  \texttt{no}. To create a cross-compiled configuration one must explicitly
  set this option to \texttt{yes}.  You will also have to set \texttt{ENDIAN}.

\item [\texttt{ENDIAN}] Explicitly sets the endianess of multi-byte types.
  Normally this is detected automatically but needs to be specified for
  cross-compilations.  Available options are \texttt{little} and \texttt{big}.
...
\item [\texttt{LD\_DEBUG\_FLAGS}]
Debug flags for the linker, their use depends on the type of debugging
being used.
Default to \texttt{CXX\_DEBUG\_FLAGS} if \verb|LD| is not set.
...
\item [\texttt{LD\_OPTIMISE\_FLAGS}]
Optimisation flags for the linker, their use depends on the type of
optimisation being used.
Default to \texttt{CXX\_OPTIMISE\_FLAGS} if \verb|LD| is not set.
...
\item [\texttt{LD\_NO\_OPTIMISE\_FLAGS}]
Optimisation flags used to indicate that no optimisation should be
performed.  These are invoked when OPTIMISE=no is used.
Default to \texttt{CXX\_NO\_OPTIMISE\_FLAGS} if \verb|LD| is not set.


\item [\texttt{OPENMP}]
Specifies if OpenMP multi-threading should be used.  The only options
currently available are \texttt{yes} and \texttt{no}.  The default is to
use no OpenMP.
...
\item [\texttt{CUCC\_OPENMP\_FLAGS}]
OpenMP flags for the CUDA compiler, their use depends on the type of
OpenMP stack being used.
...
type of OpenMP stack being used.

\item [\texttt{F77\_OPENMP\_FLAGS}]
Ignored.

\item [\texttt{LD\_OPENMP\_FLAGS}]
OpenMP flags for the linker, their use depends on the type of OpenMP
stack being used.
Default to \texttt{CXX\_OPENMP\_FLAGS} if \verb|LD| is not set.
...
\item [\texttt{LD\_PROFILE\_FLAGS}]
  Profile flags for the linker, their use depends on the type of
  profiling being used.
  Default to \texttt{CXX\_PROFILE\_FLAGS} if \verb|LD| is not set.
...
\item [\texttt{LD\_WARN\_FLAGS}]
Warning flags for the linker, their use depends on the type of
warnings used during linking (Section \ref{sec:gmopfobuco}).
Default to \texttt{CXX\_WARN\_FLAGS} if \verb|LD| is not set.
...
Setting \var{CCTK\_TESTSUITE\_RUN\_TESTS} to a space separated list of
\var{thorn} and \var{thorn/test} entries lets you choose which tests to run.

Setting \var{CCTK\_TESTSUITE\_PARALLEL\_TESTS} to a non-zero value lets
...
Setting \var{TESTS\_DIR} lets you choose the directory where tests are run
in, by default this is the \texttt{TEST} directory in the Cactus main
directory.

Setting \var{CCTK\_TESTSUITE\_RUN\_PROCESSORS} chooses the number MPI
processes (ranks) to use. It defaults to $2$.

Setting \var{CCTK\_TESTSUITE\_RUN\_COMMAND} sets the command to execute to
start a single test. Cactus will replace the strings \var{\$nprocs},
\var{\$exe}, and \var{\$parfile} by the requested number of MPI processes,
...
Setting \var{PROMPT} to \texttt{no} skips the interactive prompts and uses
...
%\item expressions: e.g. \verb=3+4*(2-foo::bar)=. Expressions are evaluated in standard
%order of precedenc (e.g. multiplication before addition). Expressions may contain other
%parameters, a few basic functions (e.g. sin, cos). See detailed description below.
%\end{itemize}
\item Variables:
