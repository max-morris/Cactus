...
Util_TableSetIntArray(handle, N_STUFF, stuff, "stuff");

/* gets N = 3, output[0] = 42.0, output[1] = 69.0 */
N = Util_TableGetRealArray(handle, N_OUTPUT, output, "stuff");
\end{verbatim}
\end{Example}
\end{ExampleSection}
\end{FunctionDescription}
...
key	& any string not containing \verb|'/'|
	  or \verb|'='| or \verb|';'| or whitespace			\\
value	& int\_value $\big|$ real\_value				\\
int\_value	& contains only characters from \verb|+-0123456789|
		  and is recognized as valid by \verb|sscanf()|
		  with a \verb|"%d"| format				\\
real\_value	& contains one or more characters not in
		  \verb|+-0123456789| and is recognized as valid
		  by \verb|sscanf()| with a \verb|"%lf"| format		\\
delimiter	& end-of-string $\big|$ \verb|';'| $\big|$ whitespace	\\
