...
\{
  ...
  INVALIDATES: [\var{variable or group list with region specification}]
...
\item[\texttt{READS/WRITES/INVALIDATES}] \texttt{READS/WRITES/INVALIDATES} are used to
  ...
  READS/WRITE/INVALIDATES directives can be applied to either a variable or a group of
  ...
  will be updated if a boundary function was registered using either
  \texttt{Boundary\_RegisterPhysicalBC} or \texttt{Driver\_RegisterPhysicalBC}. The
  arguments to both functions are the same:

\begin{verbatim}
Driver_RegisterPhysicalBC(CCTK_POINTER cctkGH,
                          phys_bc_fn_ptr function_pointer,
                          CCTK_STRING bc_name)
\end{verbatim}

The function pointer (\texttt{phys\_bc\_fn\_ptr}) takes the following arguments:
...
\begin{itemize}
\item \texttt{cctkGH}	pointer to the grid hierarchy
\item \texttt{num\_vars}	number of entries passed in the following three arrays
\item \texttt{var\_indices}	array of \texttt{num\_vars} variable indices selected for this boundary condition
\item \texttt{faces}	array of \texttt{num\_vars} faces specifications (see section 2.4)
\item \texttt{widths}	array of \texttt{num\_vars} boundary widths (see below)
\item \texttt{table\_handles}	array of \texttt{num\_vars} table handles
\end{itemize}

The table handles hold extra arguments for each application of the boundary condition. The four arrays are sorted first on table handle, and then on variable index. This way variables which have precisely the same boundary condition are grouped together, and within this grouping variables are sorted by index, so that variable groups are stored together. In many cases this sorting will allow a more efficient implementation of the boundary condition. (At the moment it is not clear how face information should be considered in the sorting, so it is not.) A null pointer can be passed for \texttt{function\_pointer}, in which case no routine is executed when \texttt{Driver\_ApplyPhysicalBCs} is called.
...
// This second version of Arguments needs
// to be present for the function-specific version
// of the DECLARE_CCTK_ARGUMENTS, i.e.
// DECLARE_CCTK_ARGUMENTS_<function name>
// See text below.
...
be declared with \texttt{DECLARE\_CCTK\_ARGUMENTS\_<function name>},
...
c     I'm very cautious, so I want to declare all variables
      ...
      DECLARE_CCTK_ARGUMENTS_MyFRoutine
...
c in the READS/WRITES clauses of the schedule.ccl declaration.
      ...
      DECLARE_CCTK_ARGUMENTS_MyNewRoutine
...
{
  DECLARE_CCTK_ARGUMENTS_MyCRoutine
  DECLARE_CCTK_PARAMETERS
