...
All capabilities have a version attached to them, so that other thorns
can depend on such a specific version. Version strings can only contain
letters, numbers, or ``.+-:'' (dot, plus, minus, colon), and have to start
with a number. Specifying a version number is optional. If none is given,
``0.0.1'' is assumed.
...
string consisting entirely of digits is determined. The numerical values of
...
\begin{alltt}
REQUIRES ExampleCapability (>=2.43.2dev)
\end{alltt}
