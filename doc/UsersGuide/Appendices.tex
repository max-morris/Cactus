...
\begin{itemize}
\item \texttt{psync\_mode=off}: Cactus behaves as it did prior to the inclusion of PreSync. READS and
...
\item \texttt{psync\_mode=warn-only}: The same as \texttt{psync\_off}, except that now, if the Driver
believes that a SYNC is required but not executed, it issues a warning.
\item \texttt{psync\_mode=mixed-warn}:
...
\item \texttt{psync\_mode=mixed-error}
In this mode, Cactus tries to honor both SYNC statements in
the schedule as well as providing automatic synchronization based on the READS/WRITES declarations.
It may, however, issue errors when encountering possible problems.
\item \texttt{psync\_mode=psync-only}:
...
\item ``Can't sync''describes a situation where synchronization is needed, but (as far as the driver knows) the interior of the grid function is not valid.
\item ``IO Thorns'' describes what the various IO thorns should do if they are asked to write out a
grid function if they find invalid data in the interior and/or exterior of a thorn.
\item ``Old Macro'' refers to the familiar \texttt{DECLARE\_CCTK\_ARGUMENTS}, which declares
all the grid functions visible to a thorn. The \texttt{DECLARE\_CCTK\_ARGUMENTS\_func\_name} macros
