...
\item[{\tt grid scalar}]
  ...
  i.e.\hbox{} it's just a number on each processor.
...
\item[{\tt NULL pointer}] C defines a ``NULL pointer'' which is guaranteed not
  ...
  constant 0 to a pointer type, e.g.\hbox{} \verb|int* ptr = 0;|.  Note
...
\end{verbatim}
  ...
  pointers, e.g.\hbox{} \verb|int* ptr = NULL;|, to emphasize to
...
which have the same cctk_levfac numbers.
