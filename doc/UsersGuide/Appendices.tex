...
\item[\texttt{READS/WRITES}] \texttt{READS/WRITES} are used to
  ...
  is required and will happen automatically if enabled with the Cactus parameter \texttt{psync\_mode}.
...
\begin{description}
\item[Driver\_RequireValidData(]~
\begin{verbatim}
    DECLARE_CCTK_ARGUMENTS,
    CCTK_INT *vars,CCTK_INT *tls,
    CCTK_INT nvars, CCTK_INT *where)
\end{verbatim}
This function specifies that for each of the \texttt{nvar} whose identifiers is given in \texttt{vars} and time levels in \texttt{tls},
data corresponding to the valid regions given by the specifier in
\texttt{where}.
%\item[Driver\_NotifyDataModified]
%\item[Driver\_GetValidRegion]
%\item[Driver\_GetValidRegion]
\end{description}
...
\item [{\tt psync\_mode}]
The \texttt{psync\_mode} parameter defines the behavior of the automatic pre-synchronization of grid functions. A summary of the different modes available is available in Table~\ref{tab:psync}.

The meaning of modes is as follows:
\begin{description}
\item[psync\_mode=off]: This is the default. Cactus behaves as it did prior to the inclusion of PreSync. READS and
...
\item[psync\_mode=warn-only] The same as \texttt{psync\_off}, except that now, if the Driver
believes that a SYNC is required but not executed, it issues a warning.
\item[psync\_mode=mixed-warn]
...
\hline
\textbf{psync\_mode} & \textbf{Synch triggered by} & \textbf{Can sync}                                                   & \textbf{Can't sync} & \textbf{IO Thorns}                                                               & \textbf{Old Macro} & \textbf{Driver BC} & \textbf{Scheduled SYNC} \\ \hline
off & schedule only & nothing & nothing & nothing & allowed & ignored & honored \\ \hline
warn-only & schedule only & warn & warn & nothing & allowed & ignored & honored \\ \hline
mixed-warn & \begin{tabular}[c]{@{}l@{}}driver where possible,\\ schedule otherwise\end{tabular} & \begin{tabular}[c]{@{}l@{}}sync if driver,\\ warn otherwise\end{tabular} & warn & \begin{tabular}[c]{@{}l@{}}warn for driver bc,\\ nothing otherwise\end{tabular} & allowed & allowed & ignored for driver bc \\ \hline
mixed-error          & \begin{tabular}[c]{@{}l@{}}driver where possible,\\ schedule otherwise\end{tabular}   & \begin{tabular}[c]{@{}l@{}}sync if driver,\\ error otherwise\end{tabular} & error                     & \begin{tabular}[c]{@{}l@{}}error for driver bc,\\ nothing otherwise\end{tabular} & allowed                           & allowed                           & ignored for driver bc  \\ \hline
psync-only & driver only & driver only & error & error & error & required & ignored \\ \hline
\end{tabular}
\caption{Summary of modes and meanings for the \texttt{psync\_mode} parameter.}
\label{tab:psync}
\end{table}
